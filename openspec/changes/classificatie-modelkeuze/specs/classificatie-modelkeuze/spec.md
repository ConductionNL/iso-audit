...
### Requirement: Tokengebruik en kosten worden vastgelegd

Elke classificatie MUST zijn tokengebruik in `classifications.usage_json` opslaan.

Het run-record MUST de totale kosten bevatten, met de peildatum van de tarieven én de
prijsgrondslag.

Rationale: de kolom en de aanroep bestonden al, maar gemeten op 2026-08-17 stond er in geen
enkele run data in — niet in `output/audit.db` en niet in de referentie-checkout. Daarmee is
"wat kost een audit" onbeantwoordbaar en is elke modelkeuze een gevoelskwestie.

#### Scenario: Run met classificaties

- **WHEN** een run classificaties heeft uitgevoerd
- **THEN** heeft elke classificatie een gevuld `usage_json`
- **AND** staat in het run-record een kostenbedrag met peildatum en grondslag
